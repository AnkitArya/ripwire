...
| **The gate suite** | `test/regression.sh`, `test/pargates.py` | 551 gate scripts plus the determinism, cache-transparency and golden contracts. |
...
naming **551 gate scripts**, all of which exist on disk.
...
- **A single round gate-count.** Two in-tree numbers disagree (`test/pargates.py`'s docstring says
  ~210; `test/argvdiffcheck.sh` says 200+), while the loop in `test/regression.sh` names 551. The
