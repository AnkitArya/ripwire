...
**Answers:** can `--recall` serve as a zero-setup knowledge base over logs, API dumps, fetched
docs, or `--help` text sitting in a scratch directory — instead of a source repo?

Yes, unmodified. `--recall` never distinguishes "a codebase" from any other directory it can
walk for markdown/text/JSON/etc — point it at a scratch dir holding a `git log`, an API
response dump, a fetched doc, or `<tool> --help` output and query it directly. No index to
build, no daemon, no mutable store between runs: the whole cost is one cold parse.

**Try it**

_The directory-as-knowledge-base pattern: --recall pointed at a 2015333-byte scratch dir of DUMPED TOOL OUTPUT (a git log, this repo's own generated command reference, --help text, an architecture doc, a fabricated JSON access log) instead of a source repo — no index to build, no daemon._
...
- KNOWN LIMIT — DELETE THIS BULLET OUTRIGHT (not just reword it) once `src/recall.h` gets a
  passage-serving fix. Today `--recall` computes a relevance ranking over a document's
  sections and then serves a document-order PREFIX, not the ranked sections — so on a dumped
  document LARGER than its computed byte share, the answer can be structurally unreachable at
  every `--max-tokens`. Measured on the run above's own corpus: the
  query's true answer (the `--field-affinity` section of the dumped `commands.md`) is ranked
  #1, selected, and still absent from the served bytes at `--max-tokens` 1500, 4000, 12000 and
  40000 alike — only front matter is ever served. Until that fix lands, point this pattern at
  dumps that individually fit inside their computed share, or split one large dump into
