...
/* ── S10 · proven, not promised ─────────────────────────────────────────── */
{
  ...
  const cards = [
    ["561 gate scripts", "the suite runs on every push — plus determinism, cache-transparency and golden contracts; the gate count itself is gated against the runner's own loop"],
...
/* ── S10b · why the claims are worth anything ───────────────────────────── */
{
  ...
  stat(s, "561", "gate scripts named by test/regression.sh — and the COUNT itself is gated against the runner's own loop, so it cannot go stale quietly",
...
/* ── S12b · the research inside ─────────────────────────────────────────── */
{
  ...
  s.addText([
    { text: "43 repositories + 69 papers folded", options: { color: TEXT, bold: true } },
...
/* ── S12c · re-derive this deck ─────────────────────────────────────────── */
{
  ...
  const claims = [
    ...
    ["561 gate scripts",                  "bash test/manifestcheck.sh"],
    ["43 repos · 69 papers · 237 surveyed","bash test/readmedriftcheck.sh"],
