...
; Stated floor: a compound `obj.count += 1` is an operator_assignment that reads AND writes; one
; capture carries one name, so it keeps the getter edge only (test/rubysettercheck.sh pins this).
(call
